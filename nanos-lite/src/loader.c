  ...
  for (int i=0; i<elf.e_phnum; i++) {
    ...
    if (ph.p_type == 1) {
      ...
      if (pcb) {
        ...
        uintptr_t set_len = ((uintptr_t)load_pg-pa_start-ph.p_filesz)%PGSIZE;
        memset((char*)(pa_start+ph.p_filesz+4), 0, set_len-4);//(uintptr_t)load_pg+PGSIZE-pa_start-ph.p_filesz);
