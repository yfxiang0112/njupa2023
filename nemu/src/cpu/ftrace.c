...
void rec_ftrace(vaddr_t addr, vaddr_t pc, uint32_t inst_val) {
	...
	if (inst_val == 0x00008067) {
        ...
        call_node *temp;
				while (st_top_call->func_ind != i) {
					call_cnt--;
					printf("%s 0x%x%*s%s [%s]\n", TRACE_STR("[MTRACE]:"), pc, call_cnt, " ", ANSI_FMT("ret ", ANSI_FG_YELLOW), funct_tab[st_top_call->func_ind].name);
          temp = st_top_call;
        ...
        free(st_top_call);
				...
				/* current return */
				call_cnt --;
				printf("%s 0x%x%*s%s [%s]\n", TRACE_STR("[MTRACE]:"), pc, call_cnt, " ", ANSI_FMT("ret ", ANSI_FG_YELLOW), funct_tab[i].name);
				return;
			}
		}
	}
	...
	for (int i=0; i<func_num; i++) {
		if (addr == funct_tab[i].addr) {
			printf("%s 0x%x%*s%s [%s @0x%x]\n", TRACE_STR("[MTRACE]:"), pc, call_cnt, " ", ANSI_FMT("call", ANSI_FG_CYAN), funct_tab[i].name, addr);
			call_cnt ++;
