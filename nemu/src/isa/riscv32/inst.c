...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; R(rd) = s->snpc;  rec_ftrace(s->dnpc, s->snpc-4, s->isa.inst.val));
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) >>1 <<1; R(rd) = s->snpc; rec_ftrace(s->dnpc, s->snpc-4, s->isa.inst.val));
