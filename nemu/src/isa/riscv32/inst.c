...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + imm; R(rd) = s->snpc;  rec_ftrace(s->dnpc, s->pc, s->isa.inst.val));
  INSTPAT("??????? ????? ????? ??? ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) >>1 <<1; vaddr_t t = s->snpc; rec_ftrace(s->dnpc, s->pc, s->isa.inst.val); R(rd) = t);

  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, if((int32_t)src1==(int32_t)src2) { s->dnpc = s->pc + imm; });
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", beq    , B, if((int32_t)src1!=(int32_t)src2) { s->dnpc = s->pc + imm; });
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(BITS(Mr(src1 + imm, 1, true), 7, 0),8));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(rd) = SEXT(BITS(Mr(src1 + imm, 2, true), 15, 0),16));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(rd) = Mr(src1 + imm, 4, true));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1, true));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(rd) = Mr(src1 + imm, 2, true));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2, true));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2, true));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + imm, 4, src2, true));
  ...
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(rd) = BITS((uint32_t)src1 * (uint32_t)src2, 31, 0));
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(rd) = (SEXT((int64_t)src1, 32) * SEXT((int64_t)src2, 32)) >> 32);
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw  , I, 
          uint32_t temp=src1; if(rd!=0){R(rd)=CSR(imm);} CSR(imm)=temp);
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, 
          uint32_t temp=src1; if(rd!=0){R(rd)=CSR(imm);} CSR(imm)= CSR(imm) | temp);

  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, 
          s->dnpc = isa_raise_intr(R(MUXDEF(__riscv_e,15,17)), s->snpc)); 
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = CSR(SR_MEPC);
          CSR(SR_MSTATUS) = (CSR(SR_MSTATUS)&0xfffffff7); 
          CSR(SR_MSTATUS) = CSR(SR_MSTATUS) | ((CSR(SR_MSTATUS)&0x80)>>4);
          CSR(SR_MSTATUS) = CSR(SR_MSTATUS) | 0x80); 
